...
服务默认跑在 `http://127.0.0.1:3001`（只监听本机，不对局域网开放）。第一次启动后，去前端「📱 WhatsApp 提醒设置」页面，会看到一个二维码，用手机 WhatsApp 的「设置 → 已连接的设备 → 连接设备」扫描它，跟连接 WhatsApp Web 网页版一样。连上之后状态会变成「已连接」。
...
| --- | --- | --- |
| `PORT` | `3001` | 后端监听端口 |
| `HOST` | `127.0.0.1` | 监听地址；改成 `0.0.0.0` 才会对局域网开放（不建议） |
| `ALLOWED_ORIGINS` | `http://localhost:8000,http://127.0.0.1:8000,null` | 允许跨域调用 `/api/*` 的前端来源，逗号分隔；`null` 对应直接双击打开 `index.html`（`file://`）的情况 |

如果你把前端跑在别的端口（比如 `python3 -m http.server 5500`），记得同步改一下 `ALLOWED_ORIGINS`，否则浏览器会因为 CORS 被拦截。
...
- **侧边栏导航**：左侧固定侧边栏切换「任务列表」「WhatsApp 提醒设置」，窄屏（手机）会自动折成顶部横向导航栏。
- **任务管理**：新增 / 编辑 / 删除，可设置标题、截止日期时间、优先级、备注；任务状态分「待办 / 进行中 / 已完成」三种。
- **三种视图**（任务列表页右上角切换）：
  - 🗂️ **卡片**：默认视图，适合快速浏览。
  - 📊 **表格**：紧凑列表，可直接在「状态」栏用下拉选单改状态。
  - 📌 **看板**：待办 / 进行中 / 已完成 三栏，可拖拽卡片换栏，也可以用卡片上的 ◀ ▶ 按钮移动（照顾无法拖拽的情况，比如触屏或自动化测试）。
- **筛选**：全部 / 今日 / 未完成 / 已完成，三种视图共用同一套筛选条件。
...
├── js/
│   ├── auth.js              # 模拟登录 / 登出
│   ├── tasks.js              # 任务增删改查、三种视图（卡片/表格/看板）渲染
...
5. **部署上线**：`server/` 目前只适合本地跑。如果要部署到云端长期运行，要考虑会话持久化、断线重连、以及不把它暴露成公开可调用的发送接口。
6. **接口鉴权**：现在只靠「监听地址 + CORS 白名单」挡住浏览器里其他网页的跨域调用，`/api/send`、`/api/logout` 本身仍然没有任何身份验证——只要能访问到 `127.0.0.1:3001`（比如同一台电脑上的其他程序）就能调用。可以练习加一个简单的本地 token（比如启动时生成、写入前端设置页手动填入，请求头里带上）。

## 注意事项

- 本模板是**教学用途**，登录机制是 Mock 的；WhatsApp 连接虽然是真的，但用的是非官方协议实现，仅适合个人自用，请勿用于生产环境或批量/商用场景。
- 后端默认只监听 `127.0.0.1`，并且用 `ALLOWED_ORIGINS` 白名单限制了能跨域调用 `/api/*` 的前端来源，降低了「浏览器里随便一个网页都能借你的 WhatsApp 发消息」的风险，但接口本身仍然**没有身份验证**——任何能直接访问到这台电脑 3001 端口的程序（不经过浏览器 CORS）都能调用，仅适合本地开发环境使用，见上面「给学员的练习方向」第 6 条。
